...
.EXAMPLE
  $app | Export-PacKitApplicationFragment -SourceFolder 'C:\src\Acme'

.EXAMPLE
  Export-PacKitApplicationFragment -Fragment $app -LiteralPath '.\out\app.xml' -WhatIf
#>
function Export-PacKitApplicationFragment {
    ...
    process {
        if (-not (Test-PacKitGuid -Value $Fragment.AppId)) {
            throw "Fragment AppId '$($Fragment.AppId)' is not a valid PacKit GUID (expected braced UPPERCASE form). Use New-PacKitApplicationFragment to create one."
        }
        ...
        if ($PSCmdlet.ParameterSetName -eq 'BySourceFolder') {
            ...
            $target = Join-Path $packitFolder ($Fragment.AppId + '.xml')
        }
        else {
            $target = $LiteralPath
        }
        ...
        if ($RelativizePaths) {
            ...
            $toEmit = ConvertFrom-PacKitXmlString -Xml (ConvertTo-PacKitXmlString -Fragment $Fragment)
            $toEmit.IconPath = Get-PacKitRelativePath -BaseDirectory $baseDir -Path $toEmit.IconPath
            $toEmit.DetectionRule.ScriptPath = Get-PacKitRelativePath -BaseDirectory $baseDir -Path $toEmit.DetectionRule.ScriptPath
            foreach ($pkg in @($toEmit.Packages)) {
                $pkg.Path = Get-PacKitRelativePath -BaseDirectory $baseDir -Path $pkg.Path
                $pkg.SourceFolder = Get-PacKitRelativePath -BaseDirectory $baseDir -Path $pkg.SourceFolder
            }
        }

        $xml = ConvertTo-PacKitXmlString -Fragment $toEmit

        if ($PSCmdlet.ShouldProcess($target, 'Write PacKit application fragment')) {
            $written = Write-PacKitBytes -Path $target -Content $xml
